...
async function renderQuotations(subpage = '') {
  ...
  if (subpage && !isNaN(subpage)) { await renderQuoteBuilder(parseInt(subpage)); return; }
  ...
  return `
    <div class="doc-card" onclick="navigate('quotations/${q.id}')">
      ...
      <div class="action-row" onclick="event.stopPropagation()">
        ...
        <button class="btn btn-ghost btn-icon" onclick="window.KwezaPDF.generatePDF('quotation',${q.id})" title="PDF">📄</button>
...
Object.assign(window.KwezaPages, {
  renderQuotations, renderQuoteBuilder, addLineItem, removeLineItem,
